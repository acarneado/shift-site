...
export const SHARE_TITLE = "Alexandre Carneado - Coach Professionnel";
...
// knowsAbout/hasCredential/areaServed : résumé machine-readable de
// l'expertise pour les moteurs génératifs (ChatGPT, Perplexity, etc.),
// distinct de la prose marketing des pages.
...
// Données structurées (JSON-LD) d'une offre : résumé structuré d'une
// prestation (nom, description, zone d'intervention), distinct de la
// prose de la page. À rendre dans le page.js de chaque offre.
export function serviceJsonLd({ name, description, routePath }) {
  return {
    "@context": "https://schema.org",
    "@type": "Service",
    serviceType: name,
    name,
    description,
    provider: { "@type": "Person", name: "Alexandre Carneado" },
    areaServed: "Île-de-France",
    url: `${SITE_URL}${routePath}`,
  };
}

// Données structurées (JSON-LD) d'une FAQ : format que les moteurs
// génératifs (ChatGPT, Perplexity, Google AI Overviews...) privilégient
// pour citer une réponse factuelle courte, plutôt qu'un paragraphe
// narratif. `items` = [{ question, answer }].
